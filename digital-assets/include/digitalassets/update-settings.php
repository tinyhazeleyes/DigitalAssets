...
require_once $_SERVER['DOCUMENT_ROOT'].'/wp-content/plugins/digital-assets/digital-assets.php';
...
if ( 'update' == $action ) {
	...
	if ( $options ) {
		...
		foreach ( $options as $option ) {
			...
			if ($option == 'digital_assets_binance_key' ) {
				$form_api_key = $_POST['digital_assets_binance_key'];
				
				if($form_api_key != get_option('digital_assets_binance_key')) {
	...
	$site_key = digital_assets::get_site_key( $site_key_encoded );
			...
			} elseif ($option == 'digital_assets_binance_secret') {
				$form_api_secret = $_POST['digital_assets_binance_secret'];
				if($form_api_secret != get_option('digital_assets_binance_secret')) {
					if (!empty($form_api_secret)) {
					    	$site_key_encoded = get_option('digital_assets_site_key_encoded');
	$site_key = digital_assets::get_site_key( $site_key_encoded );
						$encrypted_api_secret  = Crypto::encrypt($form_api_secret, $site_key);
						update_option('digital_assets_binance_secret_encrypted', $encrypted_api_secret);
						update_option('digital_assets_binance_secret', $encrypted_api_secret);
					}
				}			
			} else {
